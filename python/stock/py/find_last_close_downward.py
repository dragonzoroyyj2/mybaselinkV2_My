# -*- coding: utf-8 -*-
"""
📉 find_last_close_downward.py (v2.6 이동평균선 5/20/60 추가 및 범례 추가, 경로/구문 오류 최종 수정)
------------------------------------------------------
✅ stdout = JSON 전용 출력
...
✅ Pathlib 경로 설정 유지 (parents[2] 고정)
✅ 차트 모드: 일봉 + 이동평균선 (5, 20, 60일) 적용 및 범례 추가
...
# =====================================================
# 📁 경로 설정 (사용자 요청에 따라 parents[2]로 고정)
# =====================================================
...
def generate_chart(symbol, start_date=None, end_date=None):
    try:
        ...
        # ⬇️⬇️⬇️ 추가된 범례 설정 코드 시작 ⬇️⬇️⬇️
        # mplfinance 0.12.0 이후 버전은 axes 튜플 대신 Axes 객체 하나를 반환할 수 있습니다.
        if isinstance(axes, (list, tuple)):
            # axes가 튜플 형태이면 첫 번째 요소를 메인 축으로 사용합니다.
            ax = axes[0]
        else:
            ax = axes

        # mplfinance의 plot 함수가 그린 선 객체들에 접근합니다.
        if hasattr(ax, 'lines'):
            lines = ax.lines
            # 각 이동평균선에 대한 레이블 설정 (순서대로 5, 20, 60일)
            if len(lines) >= 3:
                # 캔들 차트 다음에 그려지는 선들이 MA 선입니다.
                lines[-3].set_label('MA 5일선')
                lines[-2].set_label('MA 20일선')
                lines[-1].set_label('MA 60일선')
                
                # 범례 표시
                ax.legend(loc='best', fontsize='small')
        # ⬆️⬆️⬆️ 추가된 범례 설정 코드 끝 ⬆️⬆️⬆️
...
if __name__ == "__main__":
    ...
    elif args.base_symbol:
         # 이 부분은 원래 코드에서 비어있었으므로, 기본 분석을 실행하도록 가정합니다.
