/* ===============================================================
   ✅ commonUnifiedList_op.js (v1.2 - 2025.11 완전 안정판)
   ---------------------------------------------------------------
...
class UnifiedList {
  ...
  _renderPagination(totalPages) {
    this._lastTotalPages = totalPages; // ✅ 리사이즈 시 참조용
  ...
  async downloadExcel() {
    ...
    const search =
      document.querySelector(this.config.searchInputSelector)?.value || "";
    const url = `${this.config.apiUrl}/excel?search=${encodeURIComponent(search)}&t=${Date.now()}`;
    ...
    try {
      ...
      overlay.style.display = "none"; // ✅ 스피너 닫기
    }
  }
